...
use Chevere\Tests\src\ControllerWithParameterWithoutAttribute;
...
use function Chevere\Router\parameterToRegex;
use function Chevere\Router\parameterToVariableRegex;
...
final class FunctionsTest extends TestCase
{
    ...
    public function testRouteDefaultRegex(): void
    {
        $route = route(
            path: '/test/{id}',
            GET: ControllerWithParameterWithoutAttribute::class,
        );
        $this->assertSame(
            '/test/{id:[^/]+}',
            strval($route->path())
    ...
    #[DataProvider('parameterToVariableRegexDataProvider')]
    public function testParameterToVariableRegex(string $pattern, ParameterInterface $parameter): void
    {
        $this->assertSame($pattern, (string) parameterToVariableRegex($parameter));
    }

    public static function parameterToVariableRegexDataProvider(): array
    {
        return [
            ['\d+', int()],
            ['\d*\.?\d*', float()],
    ...
    #[DataProvider('parameterToRegexDataProvider')]
    public function testParameterToRegex(string $pattern, ParameterInterface $parameter): void
    {
        $this->assertSame($pattern, (string) parameterToRegex($parameter));
    }

    public static function parameterToRegexDataProvider(): array
    {
        return [
            ['/^\d+$/', int()],
            ['/^\d*\.?\d*$/', float()],
            [StringParameterInterface::PATTERN_DEFAULT, string()],
        ];
    }
}
